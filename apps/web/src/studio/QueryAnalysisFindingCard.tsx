...
export function QueryAnalysisFindingCard({ finding, onSelectNode, selectedNodeId }: QueryAnalysisFindingCardProps) {
    ...
    return (
        <article className={`rounded-lg border p-4 ${activeNodeId === selectedNodeId ? 'border-cyan-400/35 bg-cyan-500/5' : 'border-white/5 bg-black/20'}`}>
            <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex flex-col items-end gap-2">
                    ...
                    {finding.focusNodeId && (
                        <button
                            type="button"
                            onClick={() => onSelectNode(finding.focusNodeId || '')}
                            className={`rounded-md border px-2 py-1 text-[10px] font-black uppercase tracking-widest transition-colors ${finding.focusNodeId === selectedNodeId ? 'border-cyan-400/40 bg-cyan-500/10 text-cyan-100' : 'border-white/10 bg-black/20 text-slate-300 hover:text-slate-100'}`}
                        >
                            {finding.focusNodeId === selectedNodeId ? 'Selected Node' : 'Focus Node'}
                        </button>
                    )}
                </div>
            </div>
            ...
            {finding.reasonablenessExplanation && (
                <p className="mt-3 rounded-lg border border-white/5 bg-white/[0.03] px-3 py-2 text-sm text-slate-300">
                    {finding.reasonablenessExplanation}
                </p>
            )}
